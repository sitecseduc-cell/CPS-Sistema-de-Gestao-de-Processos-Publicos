...
export default function Layout() {
  ...
  // Função auxiliar para verificar se o link é o atual (para pintar de azul)
  const isActive = (path) => {
  ...
  // Componente interno do Item do Menu (Adaptado para usar Link)
    ...
    <div className="flex h-screen bg-slate-50 font-sans overflow-hidden">
      ...
      <aside className="w-72 bg-slate-900 text-white flex flex-col hidden md:flex shadow-2xl z-50">
        ...
        <nav className="flex-1 p-4 overflow-y-auto custom-scrollbar">
          ...
          <SidebarGroup title="Inscrições & Candidatos">
            ...
            <SidebarItem icon={User} label="Visualizar Candidato" to="/visualizar" />
          </SidebarGroup>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50">
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-slate-50/50">
          {/* O Outlet é substituído pelo conteúdo da página atual */}
          <Outlet /> 
        </div>
